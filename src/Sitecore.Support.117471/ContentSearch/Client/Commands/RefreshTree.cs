...
namespace Sitecore.Support.ContentSearch.Client.Commands
{
  ...
  {
    private const string ReindexTreeFailed =
    ...
    private static Item GetItemByUri(string uri)
    {
      ...
      Database db = ContentSearchManager.Locator.GetInstance<Sitecore.Abstractions.IFactory>().GetDatabase(itemUri.DatabaseName);
